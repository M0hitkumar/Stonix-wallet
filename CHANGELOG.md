...
- Docker: Update to Brocoin Core v22.0 and c-lightning v0.10.2
...
- docker: Update to Debian 11, Brocoin Core v0.21.0, c-lightning v0.10.1 and nodejs v16.8
...
- Docker: Update Brocoin Core to v0.19.1 and c-lightning to v0.8.1
...
- Detect if the "base directory" is provided and default to the brocoin mainnet network subdirectory within in. (#130)

  This allows users to start stonix using the existing command they use (with i.e. `--ln-path ~/.lightning`)
...
- Docker: Update to Brocoin Core v0.19.0.1 and c-lightning v0.7.3
...
- Docker: Add standalone arm32v7 and arm64v8 images (#93)
  ...
  Available using the new multiarch image at `shesek/stonix-wallet:standalone` and `shesek/stonix-wallet:VERSION-standalone`,
  or directly using `shesek/stonix-wallet:standalone-arm32v7` and `shesek/stonix-wallet:VERSION-standalone-arm32v7`.

  Thanks @NicolasDorier!

- Docker: Upgrade to Brocoin Core v0.18.1 and c-lightning v0.7.2
...
- Security: Make npm-shrinkwrap actually ship with the npm package ([5827098ec](https://github.com/shesek/stonix-wallet/commit/5827098ecc301e449e58e6077b0abf4e63aa9635))

- Fix: Make QR scanner work in dark themes (a regression introduced in v0.2.6, #91)

- Fix: Make the "Open wallet" link (brocoin:<addr>) work in the Android app
...
- Changed the way the balance shown in the top-right corner is calculated. See:
  ...
  https://github.com//stonix-wallet/issues/62

  https://github.com/M0hitkumar/stonix-wallet/commit/9b500c48c4c32fd61a11995410f17adf9f6cdf8d
...
- Enable cookie file by default, persists random login credentials to `~/.stonix-wallet/cookie`.
...
- docker: Update Brocoin Core to v0.17.1
...
- Add `--cookie-file/-C <path>` to persist randomly generated login credentials to `<path>`
  (similar to the cookie file feature in brocoind)

- Add `--public-url` CLI option

- Add standalone docker image variant available as
  shesek/stonix-wallet:x.y.z-standalone and shesek/stonix-wallet:standalone
...
- Now available on Google Play: https://play.google.com/store/apps/details?id=com.stonix.wallet
...
- Docker: update c-lightning and Brocoin Core
...
- Docker: Update to Brocoin Core v0.16.3 (e718bf8324599)

- Docker: Rename {STONIX,BROCOIND}_OPTS to {STONIX,BROCOIND}_OPT (#18, 4a66cb0b6c24)

- Electron: Rename desktop app executable to `stonix-desktop`, to prevent confusion with the `stonix-wallet` server daemon
  (https://github.com/M0hitkumar/stonix-wallet/issues/24#issuecomment-421798958, 9f81f14c9344)
