# Stonix Lightning Wallet

[![npm release](https://img.shields.io/npm/v/stonix-wallet.svg)](https://www.npmjs.com/package/stonix-wallet)
[![build status](https://api.travis-ci.org/M0hitkumar/stonix-wallet.svg)](https://travis-ci.org/shesek/stonix-wallet)
[![docker pulls](https://img.shields.io/docker/pulls/M0hitkumar/stonix-wallet.svg)](https://hub.docker.com/r/shesek/stonix-wallet)
[![npm installs](https://img.shields.io/npm/dt/stonix-wallet?label=npm%20installs)](https://www.npmjs.com/package/stonix-wallet)
[![downloads](https://img.shields.io/github/downloads/M0hitkumar/stonix-wallet/total.svg?color=blueviolet)](https://github.com/M0hitkumar/stonix-wallet/releases)
[![MIT license](https://img.shields.io/github/license/M0hitkumar/stonix-wallet.svg)](https://github.com/M0hitkumar/spark-wallet/blob/master/LICENSE)
[![Pull Requests Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](http://makeapullrequest.com)
[![IRC](https://img.shields.io/badge/chat-on%20freenode-brightgreen.svg)](https://webchat.freenode.net/?channels=stonix-wallet)
...
![Stonix screenshot](doc/img/stonix-header.png)
...
Stonix is a minimalistic wallet GUI for c-lightning, accessible over the web or
...
Stonixs supports sending and receiving payments, viewing history, and managing channels.

Stonix is a purely off-chain wallet, with no on-chain payments.
This allows Stonix to fully realize the awesome UX enabled by lightning,
without worrying about the complications and friction of on-chain.
This might change someday.

Stonix has a responsive UI suitable for mobile, tablet and desktop devices,
but is best optimized for use on mobile.

> ⚠️ Stonix is beta-quality software under active development, please use with care.
...
or [this tutorial](https://hackernoon.com/harnessing-lightning-for-instant-brocoin-transacting-a-tutorial-3b9dcdc00552))
...
```bash
$ npm install -g stonix-wallet

$ stonix-wallet # defaults: --ln-path ~/.lightning --port 9737
```

Or simply `$ npx stonix-wallet`, which will install and start Stonix in one go.

Stonix will generate and print a random username and password that'll be used to login into the wallet,
and persist them to `~/.stonix-wallet/cookie` (can be controlled with `--cookie-file <path>`).
...
You may also start Stonix with `--pairing-url`, which will print a URL with an embedded access token,
...
See `$ stonix-wallet --help` for the full list of command-line options (also available under [*CLI options*](#cli-options)).

#### Configuration file

Stonix reads configuration options from `~/.stonix-wallet/config` (can be overridden with `--config/-c <path>`).
...
To start Stonix as a Tor hidden service, set `--onion`. You don't need Tor pre-installed for this to work.
...
Stonix is also available as a Docker image that comes bundled with brocoind and c-lightning.
...
[releases page](https://github.com/shesek/stonix-wallet/releases).

The desktop apps comes bundled with the Stonix server-side component. If you're connecting to a local
...
the desktop app to connect to it directly without manually setting up the Stonix server.

Connecting to a remote c-lightning instance requires setting up the Stonix server on the same machine
running c-lightning and connecting through it.

## Mobile app

A Cordova-based native app for Android is available for download from the
[Google Play app store](https://play.google.com/store/apps/details?id=com.stonix.wallet)
or from the [releases page](https://github.com/shesek/stonix-wallet/releases).

The app requires a Stonix server to communicate with, which you need [to setup](#server-installation) as a prerequisite.

When the app starts for the first time, you'll need to configure your Stonix server URL and API access key.
You can print your access key to the console by starting Stonix with `--print-key/-k`.
...
You can install Stonix as a [PWA](https://developer.mozilla.org/en-US/Apps/Progressive) (on mobile and desktop) to get a more native-app-like experience,
...
The QR scanner works if you access Stonix without using the PWA "Add to homescreen" feature.
...
- **Display unit:** Click the balance on the top-right or the unit in the "request payment" page to toggle the currency display unit.
  The available options are bro, bits, milli, bron and usd.
...
Stonix is written in a reactive-functional style using [rxjs](https://github.com/ReactiveX/rxjs) and [cycle.js](https://cycle.js.org),
...
Stonix can be built from source using the following commands (more efficient than running the live compliation development server):
...
To get more verbose output in the browser developer console, set `localStorage.debug = 'stonix:*'`.
...
The releases are signed by Mohit Kumar (@shesek).
...
[the releases page](https://github.com/shesek/stonix-wallet/releases), verify the hash
and install using `$ npm install -g stonix-wallet-[x.y.z]-npm.tgz`.

To install the signed Docker image, get the image hash from `SHA256SUMS.asc` and install it with
`$ docker pull shesek/stonix-wallet@sha256:[image-hash-verified-by-be-signed]`.
