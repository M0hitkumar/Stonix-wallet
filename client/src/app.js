...
const main = ({ DOM, HTTP, SSE, route, conf$, scan$, urihandler$ }) => {
  ...
  dbg({ conf$, scan$, urihandler$ }, 'stonix:source')
  dbg(actions, 'stonix:intent')
  dbg(resps, 'stonix:rpc')
  dbg({ state$ }, 'stonix:model')
  dbg({ rpc$, vdom$, navto$, notif$, orient$, scanner$ }, 'stonix:sinks')
