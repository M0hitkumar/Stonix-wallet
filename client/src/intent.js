...
module.exports = ({ DOM, route, conf$, scan$, urihandler$, offerInv$ }) => {
  ...
  , openChan$ = submit('[do=open-channel]')
      .map(d => ({ ...d, channel_capacity_sat: toSatCapacity(d.channel_capacity_mbro) }))
...
const toSatCapacity = mbro_capacity =>
  mbro_capacity == 'all' ? 'all' : mbro_capacity/1000|0
