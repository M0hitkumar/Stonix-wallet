...
const mbrobron = big(100000000000) // mbro in 1 bron

const
  sumChans = chans =>
    chans.filter(c => c.chan.state === 'CHANNELD_NORMAL')
         .reduce((T, c) => T + c.chan.mbronees_to_us, 0)

, sumOuts = outs =>
    outs.filter(o => o.status === 'confirmed')
        .reduce((T, o) => T + o.value*1000, 0)

, fmtAlert = (s, unitf) => s.replace(/@\{\{(\d+)\}\}/g, (_, mbro) => unitf(mbro))
...
, units    = 'bro bits milli BRON USD'.split(' ')
, unitprec = { bro: 3, bits: 5, milli: 8, BRON: 11, USD: 6 }
, unitrate = { bro: 0.001, bits: 0.00001, milli: 0.00000001, BRON: 0.00000000001 }
...
module.exports = ({ dismiss$, togExp$, togTheme$, togUnit$, page$, goHome$, goRecv$, goChan$, confPay$
  ...
  , mbrousd$ = brousd$.map(rate => big(rate).div(mbrobron)).startWith(null)
  , rate$    = O.combineLatest(unit$, mbrousd$, (unit, mbrousd) => unitrate[unit] || mbrousd)
  , unitf$   = O.combineLatest(unit$, mbrousd$, unitFormatter)
  ...
  , alert$ = O.merge(
    ...
    , paySent$.map(p  => [ 'success', `Sent payment of @{{${p.mbronees}}}` ])
    , funded$.map(c   => [ 'success', `Opening channel for @{{${c.chan.mbronees_total}}}, awaiting on-chain confirmation` ])
    ...
    // mbro format  amounts in messages
  ...
  , cbalance$ = O.merge(
    ...
    , incoming$.map(inv => N => N + inv.mbronees_received)
    , paySent$.map(pay => N => N - pay.mbronees_sent)
  ...
  , feed$ = O.combineLatest(freshInvs$, freshPays$, (invoices, payments) => [
    ...
    , ...payments.map(p => [ 'out', p.created_at, p.mbronees, p ])
    ].sort((a, b) => b[1] - a[1]))
  ...
  , amtMbro$ = amtVal$.withLatestFrom(rate$, (amt, rate) => amt && rate && big(amt).div(rate).toFixed(0) || '')
                      .merge(page$.mapTo(null)).startWith(null)
  , amtData$ = combine({
      mbronees: amtMbro$
    , amount:   unit$.withLatestFrom(amtMbro$, rate$, (unit, mbro, rate) => formatAmt(mbro, rate, unitprec[unit], false))
  ...
  dbg({ loading$, connected$, alert$, rpcHist$, freshPays$, freshInvs$, feed$, feedStart$, feedActive$ }, 'stonix:model')
  dbg({ error$ }, 'stonix:error')
  dbg({ savedConf$, conf$, expert$, theme$, unit$, conf$ }, 'stonix:config')
  ...
  , mbrousd$, brousd$: brousd$.startWith(null)
  }).shareReplay(1)
}
...
, mbronees: inv.custom_mbro || inv.mbronees
, mbronees_sent: 0
, destination: inv.payee || inv.node_id || inv.destination
, ...only(inv, 'payment_hash', 'description', 'offer_id', 'vendor', 'quantity', 'payer_note' )
})

const unitFormatter = (unit, mbrousd) => (mbro, as_alt_unit=false, non_breaking=true) => {
  const unit_d = !as_alt_unit ? unit : (unit == 'USD' ? 'bro' : 'USD')
  const unit_rate = unit_d == 'USD' ? mbrousd : unitrate[unit_d]
  ...
  return `${unit_rate ? formatAmt(mbro, unit_rate, unit_prec) : 'n/a'}${separator}${unit_d}`
}
