...
// RPC commands prefixed with an '_' are custom extensions provided by the Stonix server
exports.parseRes = ({ HTTP, SSE }) => {
  const reply = category => dropErrors(HTTP.select(category))

  dbg({ reply$: reply().map(r => [ r.request.category, r.body, r.request ]) }, 'stonix:reply')
...
, confPay$.map(pay    => [ '_pay',             [ pay.paystr, pay.custom_mbro ], { pay, bg: true } ])
, newInv$.map(inv => !inv.reusable_offer
                       ? [ 'invoice',          [ inv.mbronees, inv.label, inv.description, INVOICE_TTL ], inv ]
                       : [ 'offer',            [ inv.mbronees, inv.description, null, inv.label ], inv ])
, offerPay$.map(pay   => [ '_fetchinvoice',    [ pay.paystr, pay.custom_mbro, pay.quantity, pay.payer_note ] ])
...
, send: { method, params }
, headers: { 'X-Requested-With': 'stonix-rpc', 'X-Access': serverInfo.accessKey }
, ctx
}))
