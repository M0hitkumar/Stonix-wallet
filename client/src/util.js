...
const reUri = /^lightning:([a-z0-9]+)|^brocoin:.*[?&]lightning=([a-z0-9]+)|^(ln[a-z0-9]+)$/i
...
export const recvAmt = ({ mbronees: expected, mbronees_received: actual }) =>
...
// Parse the `bro`-suffixed amount string fields into numbers
export const parsePayment = p => ({
  ...p
, mbronees: p.mbronees != null ? p.mbronees : p.amount_mbro ? +p.amount_mbro.slice(0, -4) : null
, mbronees_sent: p.amount_sent_mbro ? +p.amount_sent_mbro.slice(0, -4) : null
})
