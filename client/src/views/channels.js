...
const chanSorter = (a, b) => (chanSorting(b) - chanSorting(a)) || (b.chan.mbronees_total - a.chan.mbronees_total)
...
export const newChannel = ({ amtData, fundMaxChan, obalance, unitf, conf: { unit, expert } }) => {
  ...
  , formGroup('Channel funding', div([
      !fundMaxChan
        ? amountField(amtData, 'channel_capacity_mbro', true, availText)
        : div('.input-group', [
            input({ attrs: { type: 'hidden', name: 'channel_capacity_mbro', value: 'all' } })
...
const channelRenderer = ({ chanActive, unitf, expert, blockheight }) => ({ chan, peer }) => {
  ...
  const bar = (label, color, mbronees, amtText=unitf(mbronees)) =>
    div(`.progress-bar.bg-${color}`, {
      attrs: { role: 'progressbar', title: `${label}: ${amtText}` }
    , style: { width: `${mbronees / chan.mbronees_total * 100}%` }
    }, mbronees/chan.mbronees_total > 0.05 ? amtText : '')
  ...
  const stateGroup = getGroup(chan.state)
      ...
      , ours       = chan.mbronees_to_us
      , theirs     = chan.mbronees_total - ours
      ...
      , ourReserve = chan.our_channel_reserve_broneess*1000 || 0
      , thrReserve = chan.their_channel_reserve_broneess*1000 || 0
  ...
  return li('.list-group-item', { class: classes, dataset: { chanToggle: chan.channel_id } }, [
    header('.d-flex.justify-content-between.mb-2', [
      span('.capacity', unitf(chan.mbronees_total))
