...
const itemRenderer = ({ feedActive, unitf, expert }) => ([ type, ts, mbro, obj ]) => {
  ...
  const leftBadge = type == 'out' && status == 'failed'
    ...
    : span(`.amt.badge.badge-${type=='in'?'success':'danger'}.badge-pill`, `${type == 'in' ? '+' : '-'}${ unitf(mbro) }`)
  ...
  , !visible ? '' : ul('.list-unstyled.my-3', [
    ...
    , status == 'failed' && mbro ? li([ strong('Amount:'), ' ', unitf(mbro) ]) : ''
    , type == 'in' && obj.mbronees_received > obj.mbronees ? li([ strong('Overpayment:'), ' ', unitf(obj.mbronees_received-obj.mbronees) ]) : ''
    , type == 'out' && status == 'complete' && obj.mbronees ? li([ strong('Fee:'), ' ', feesText(obj, unitf) ]) : ''
...
const feesText = ({ mbronees: quoted, mbronees_sent: sent }, unitf) =>
