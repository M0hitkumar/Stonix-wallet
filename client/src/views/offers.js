...
const offerPay = offer => ({ unitf, amtData, offerPayQuantity, conf: { expert } }) =>
  ...
  ,
    ...
    ? p('.toggle-unit', [ offer.quantity_min ? 'Price per unit: ' : 'Amount: '
      , fmtSatAmountWithAlt(offer.mbronees, unitf) ])
    ...
    ? div('.form-group', [
      ...
      , small('.form-text.text-muted', [ strong('Informative only.'), ' The final BRON amount will be displayed for confirmation on the next screen.' ])
      ])
  ...
  , div('.form-buttons', [
    ...
    , div([
        ...
        , offer.mbronees ? `Pay ${unitf(mul(offer.mbronees, offerPayQuantity))}`
...
const offerRecv = offer => ({ unitf, conf: { expert} }) =>
  ...
  , p([ 'You were offered a payment of ', strong('.toggle-unit', unitf(offer.mbronees)), '. Do you accept it?' ])
...
export const localOffer = offer => qrinv(offer).then(qr => ({ unitf, conf: { expert } }) =>
  div('.local-offer-recv', [
    div('.row', [
      ...
      , p(`You can receive multiple payments${offer.mbronees != 'any'?` of ${unitf(offer.mbronees)} each`:''} using the reusable BOLT12 offer:`)
...
const mul = (mbronees, quantity=1) =>
  quantity == 1 ? mbronees : Big(mbronees).mul(quantity).toFixed(0)
