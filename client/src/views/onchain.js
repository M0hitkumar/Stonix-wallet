...
const addrQr = (address, type) => qruri(`brocoin:${type == 'bech32' ? address.toUpperCase() : address}`)
...
export const deposit = addresses => async ({ funds, obalance, depositAddrType: type, unitf, conf: { expert } }) =>
  ...
  , div('.my-4.text-center', [
      a('.btn.btn-primary.btn-lg.mb-1', { attrs: { href: `brocoin:${addresses[type]}` } }, 'Open wallet')
    , ' '
