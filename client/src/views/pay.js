...
const hasCam = (navigator.mediaDevices && navigator.mediaDevices.getUserMedia)
    ...
    , camSuggest = hasCam ? null
      ...
      : isPWAiOS ? 'PWAs on iOS cannot access WebRTC. Try opening Stonix in your regular Safari browser without using "Add to homescreen".'
...
const confirmPay = payreq => ({ unitf, amtData, conf: { expert } }) => {
  ...
  // If the original offer had a fiat-denominated amount, exclude the 'mbro' field
  // from being displayed in the changes list. The brocoin and fiat amounts are
  // displayed separately.
  if (payreq.offer && !payreq.offer.mbronees && payreq.offer.amount && payreq.mbronees) {
    delete payreq.changes.mbro
  }
  ...
  ,
    // Brocoin-denominated amount that was previously displayed in fiat
    (payreq.mbronees && payreq.offer && payreq.offer.amount)
    ? div('.form-group', [
        p('.mb-0.toggle-unit', [ 'Final amount: ', fmtSatAmountWithAlt(payreq.mbronees, unitf) ])
      , div('.form-text.text-muted', [ 'Quoted as: ', strong(fmtOfferFiatAmount(payreq.offer, payreq.quantity)) ])
      ])

    // Brocoin denominated amount
    : payreq.mbronees
    ? p('.toggle-unit', [ 'Amount: ', fmtSatAmountWithAlt(payreq.mbronees, unitf) ])

    // Amount chosen by the payer
    : formGroup('Enter amount to pay:', amountField(amtData, 'custom_mbro', true))
  ...
  , ...(payreq.changes && Object.keys(payreq.changes).length > 0 ? [
    ...
    , ul([
       payreq.changes.mbro ? li([ 'The amount was changed from ', ...displayAmountChange(payreq, unitf) ]) : ''
  ...
  , div('.form-buttons.mt-4', [
    ...
    , button('.btn.btn-lg.btn-primary.mb-1', { attrs: { type: 'submit' } }
      , payreq.mbronees ? `Pay ${unitf(payreq.mbronees)}` : 'Send Payment')
    , ' '
...
const displayAmountChange = ({ mbronees: final, changes }, unitf) => {
  const original = +changes.mbro.slice(0, -4)
