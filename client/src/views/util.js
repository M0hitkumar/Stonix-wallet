...
export const amountField = ({ mbronees, amount, step, unit }, mbroField, required, placeholder) =>
  div('.input-group', [
    input({ attrs: { type: 'hidden', name: mbroField }, props: { value: mbronees } })
...
export const fmtSatAmountWithAlt = (mbronees, unitf) => span([
  strong(unitf(mbronees))
, fmtNullable(unitf(mbronees, true), amt => small(` (${amt})`))
])
...
export const getPricePerUnit = ({ mbronees, quantity }) =>
  Big(mbronees).div(quantity).toFixed(0)
