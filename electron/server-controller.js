...
const store = new Store({ name: 'stonix-server' })
...
function startServer(lnPath) {
  ...
  proc.on('error', err => console.error('Stonix server error', err.stack || err))
  proc.on('message', m => console.log('Stonix server msg', m))
  proc.on('exit', code => console.log('Stonix server exited with status', code))
...
function stopServer() {
  if (proc) {
    console.log('Stopping embedded Stonix server')
