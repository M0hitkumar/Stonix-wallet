...
gh_repo=shesek/stonix-wallet
docker_name=shesek/stonix-wallet
...
[[ -z "$ANDROID_RELEASE_CONFIG" && -f ../stonix-signing-keys/build.json ]] && ANDROID_KEYS=`pwd`/../stonix-signing-keys

echo -e "Building Stonix v$version\n\n$changelog\n\n"

# Build NPM, Electron and Cordova dist files
if [[ -z "$SKIP_BUILD" ]]; then
  # clean up previous builds
  rm -rf docker-builds stonix-wallet-*-npm.tgz dist electron/dist cordova/platforms/android/app/build/outputs/apk
  ...
  if [[ -z "$NO_DOCKER_BUILDER" ]]; then
    ...
    mv docker-builds/stonix-wallet-*-npm.tgz .
